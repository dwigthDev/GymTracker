...
#con esta funcion se genera un archivo JSON que guarda los datos
def CrearArchivo(usuario): 
    with open ("RutinasUsuario.json",'w') as database:
        rutinas_de_usuario = usuario["rutinas"]
        json.dump(rutinas_de_usuario, database, indent=4)
        print("\n***Guardado correctamente***\n")
        time.sleep(1)
        
def LeerArchivo():#esta funcion se encarga de leer el archivo generado 
    try:
        print("\n")
        with open ("RutinasUsuario.json",'r') as database:
...
def opciones_de_usuario(usuario):
    while True:
        try:
            ...
            op = int(input(f"Ingrese la opción que desea ejecutar\n1. Crear una rutina\n2. Usar una rutina\n3. Crear y guardar archivo con rutinas\n4. Leer Archivo de Rutinas.\n5. Salir\n---> "))
            if op == 1:
                crearRutina(usuario)
            elif op == 2:
                usarRutina(usuario)     
